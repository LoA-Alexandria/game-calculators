...
3. Add or replace the item in the `guides` section in `lib/navigation.ts`,
   ...
   `coreElements`; placement guides (water supply, hero layouts) use `layouts`.
...
to `GuideArticle`, chosen by a field only that guide has: `GoddessesGuide`
(`phases`) and `HeroLayoutsGuide` (`builds`). Keep `sections` and `note` in
those entries too, since the editor reads them. In the Hero layouts guide,
