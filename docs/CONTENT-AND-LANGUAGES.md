...
3. the pictures to delete from `public/heroes/` because the draft no longer
   uses them, and
4. one `heroTexts` block per dictionary, to paste under
   `guideEntries.heroes`. It is keyed by hero id and holds only the
   translations that are filled in; English stays empty because the JSON is
   English.
