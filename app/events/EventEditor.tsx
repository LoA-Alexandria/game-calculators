...
/**
 * Produces one row for `lib/content/events.ts`.
 *
 ...
 * row is what makes the event visible to everyone.
 */
export function EventEditor() {
  ...
  const [kind, setKind] = useState<EventKind>("routine");
  ...
  }, [name, kind, start, duration, repeat, interval, weekdays, dayOfMonth, until]);
  ...
  const entry = useMemo(() => {
    ...
    return [
      ...
      `    name: (t) => t.eventEntries.${draft.id.replace(/-([a-z])/g, (_, c: string) => c.toUpperCase())}.name,`,
      `    summary: (t) => t.eventEntries.${draft.id.replace(/-([a-z])/g, (_, c: string) => c.toUpperCase())}.summary,`,
      `  },`,
      ``,
      `// and in every dictionary, under eventEntries:`,
      `// ${draft.id.replace(/-([a-z])/g, (_, c: string) => c.toUpperCase())}: { name: ${JSON.stringify(name || "…")}, summary: ${JSON.stringify(summary || "…")} },`,
    ].join("\n");
  }, [draft, name, summary]);

  const weekdayNames = useMemo(() => {
    const format = new Intl.DateTimeFormat(locale, { weekday: "short", timeZone: "UTC" });
    // 2026-09-13 is a Sunday, so index 0..6 lines up with getUTCDay()
  ...
  return (
    <section className="panel" style={{ marginTop: 24 }}>
      <h2>{t.events.editorTitle}</h2>
      <p>{t.events.editorLede}</p>

      <div className="notice notice-info" style={{ marginBottom: 18 }}>
        <InfoIcon className="icon" />
        <div><p>{t.events.editorOutputNote}</p></div>
      </div>
