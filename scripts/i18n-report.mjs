 ...
 * how many heroes, painting sets, and plays have wording in each language.
 */
...
const CATALOGS = /(?:^|\.)(heroTexts|playTexts|catalogTexts)(?:\.|$)/;
...
for (const { code, label } of LOCALES) {
  ...
  console.log(`  hero wording: ${heroTexts}/${HEROES.length} heroes · painting sets: ${sets}/${PAINTING_SETS.length} · paintings: ${paintingTexts}/${paintings} · plays: ${plays}/${THEATER_PLAYS.length}`);
